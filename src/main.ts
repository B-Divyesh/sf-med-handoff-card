...
function historySection() {
  ...
  return `<section class="history" aria-labelledby="history-heading"><p class="eyebrow">Change record</p><h2 id="history-heading">${heading}</h2>${events.length ? `<ol>${events.map(event => event.html).join('')}</ol>` : '<p class="quiet">No dose states or regimen changes were recorded for this date.</p>'}</section>`
}
...
function regimenChangeRow(change: AppData['regimenChanges'][number]): string {
  ...
  return `<li class="regimen-change"><b>Regimen changed</b> ${esc(change.medicationName)} ${time}<span>Was ${esc(change.previous.dose)} · ${esc(change.previous.slots.join(', '))}. Now ${esc(change.next.dose)} · ${esc(change.next.slots.join(', '))}.</span></li>`
}
...
document.addEventListener('click', async event => {
  ...
  if (action === 'stop-med') { const med = data.medications.find(m => m.id === target.dataset.id); if (med && confirm(`Stop showing ${med.name} in the current regimen? Existing dose history will stay.`)) { const changedAt = new Date().toISOString(); const stopped = { ...med, active: false, changedAt }; const change = { id: crypto.randomUUID(), medicationId: med.id, medicationName: med.name, previous: { dose: med.dose, instructions: med.instructions, slots: med.slots, active: med.active }, next: { dose: stopped.dose, instructions: stopped.instructions, slots: stopped.slots, active: stopped.active }, changedAt }; message = `${med.name} moved out of the current regimen. Its history stays in this record.`; await save({ ...data, medications: data.medications.map(m => m.id === med.id ? stopped : m), regimenChanges: [...data.regimenChanges, change] }) } }
