...
function htmlShell(content: string) {
  return `<header class="masthead"><a class="brand" href="/" aria-label="Med Handoff Card home"><span class="brand-mark">MH</span><span>Med Handoff<br>Card</span></a><nav aria-label="Primary"><a href="/" ${currentPath() === '/' ? 'aria-current="page"' : ''}>Board</a><a href="#regimen">Regimen</a><a href="#tools">Tools</a></nav><button class="theme" type="button" data-action="theme" aria-label="Switch to ${theme === 'dark' ? 'light' : 'dark'} appearance">${theme === 'dark' ? '☼' : '◐'} <span>${theme === 'dark' ? 'Light' : 'Night'}</span></button></header><main id="main">${content}</main><footer><span>Private by design · stored on this device</span><span><a href="/privacy">Privacy</a> <a href="/terms">Terms</a></span><small>Artwork generated for Med Handoff Card; no patient data leaves this device.</small></footer><div class="toast" aria-live="polite">${updateReady ? 'An update is ready. ' : ''}${message}${updateReady ? ' <button data-action="update">Reload</button>' : ''}</div>`
}
...
function emptyBoard() {
  return `<section class="empty-state"><div><p class="eyebrow">Private caregiver record</p><h1>Make the next handoff unambiguous.</h1><p>Start with the current medication list. Then the board will show each scheduled dose as <strong>Taken</strong>, <strong>Held</strong>, or <strong>Unknown</strong>.</p><button class="primary" data-action="add-med">Add first medication</button><p class="quiet">Stored only in this browser. Export a copy whenever you need one.</p></div><img src="${handoffTray}" width="900" height="900" alt="Dithered illustration of a pill organizer, blank handoff card, and pencil." fetchpriority="high" decoding="async"></section>${tools()}`
}
...
function paid() {
  const license = localStorage.getItem('sb_license:med-handoff-card')
  return `<section class="paid" aria-labelledby="plus-heading"><div><p class="eyebrow">Optional, one-time unlock</p><h2 id="plus-heading">Med Handoff Card Plus</h2><p>Keep separate named profiles on one shared device. $9 once; core dose records, printing, and exports stay free.</p></div><div>${license ? '<p class="license-ok">License saved on this device.</p>' : '<a class="outline link-button" href="https://api.sociobot.in/api/v1/products/med-handoff-card/checkout">Buy Plus — $9 once</a><button class="text-button" data-action="restore">Have a license? Restore it</button>'}</div><small>Sociobot / Dodo is merchant of record. Refunds revoke the license automatically.</small></section>`
}

function legal(kind: 'privacy' | 'terms') {
  const title = kind === 'privacy' ? 'Privacy, plainly.' : 'Terms for a practical handoff.'
  const text = kind === 'privacy' ? `<p>Med Handoff Card stores your medications, dose states, notes, and optional license token in this browser’s local storage. The app does not run analytics, create an account, or send your health information to us.</p><p>Printing, QR sharing, exporting, importing, and buying Plus are actions you choose. QR data is not encrypted; encrypted export uses your passphrase locally and we cannot recover it. Clear your browser data to remove local records.</p>` : `<p>This app is a personal caregiver record, not medical advice, a drug-interaction checker, or an emergency service. Confirm confusing, changed, or held medication instructions with the prescribing clinician or pharmacist.</p><p>You are responsible for protecting printed, exported, and QR-shared information. Plus is a one-time license managed by Sociobot / Dodo. Use of the app is provided as-is to the extent permitted by law.</p>`
...
function render() {
  ...
  app.innerHTML = htmlShell(path === '/privacy' ? legal('privacy') : path === '/terms' ? legal('terms') : board())
}
...
async function verifyLicense(token: string) { try { const cacheKey = 'sb_license_verdict:med-handoff-card'; const cached = JSON.parse(localStorage.getItem(cacheKey) || '{}'); if (cached.checked && Date.now() - cached.checked < 86400000) return; const response = await fetch(`https://api.sociobot.in/api/v1/products/med-handoff-card/verify?license=${encodeURIComponent(token)}`); const verdict = await response.json(); localStorage.setItem(cacheKey, JSON.stringify({ ...verdict, checked: Date.now() })); if (!verdict.valid) { localStorage.removeItem('sb_license:med-handoff-card'); message = 'License is no longer active. You can buy Plus again when ready.'; render() } } catch { /* offline: preserve optimistic local unlock */ } }
...
document.addEventListener('click', async event => {
  ...
  if (action === 'restore') { const token = prompt('Paste your Med Handoff Card Plus license.'); if (token?.trim()) { localStorage.setItem('sb_license:med-handoff-card', token.trim()); message = 'License saved. Checking it in the background.'; render(); void verifyLicense(token.trim()) } }
...
async function init() { try { data = await loadData() } catch { message = 'Local storage is unavailable. Your changes may not persist.' }; const params = new URLSearchParams(location.search); const license = params.get('license'); if (license) { localStorage.setItem('sb_license:med-handoff-card', license); history.replaceState({}, '', location.pathname); message = 'Plus license saved.' } render(); const saved = localStorage.getItem('sb_license:med-handoff-card'); if (saved) void verifyLicense(saved); if ('serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js').then(registration => { const check = () => { if (registration.waiting && navigator.serviceWorker.controller) { updateReady = true; render() } }; check(); registration.addEventListener('updatefound', () => registration.installing?.addEventListener('statechange', check)); navigator.serviceWorker.ready.then(ready => ready.active?.postMessage({ type: 'cache-assets', assets: performance.getEntriesByType('resource').map(entry => entry.name) })) }).catch(() => undefined) }
void init()
