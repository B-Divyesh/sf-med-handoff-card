...
const BUILD_ID = '2026.08.29-polish.5'
...
function htmlShell(content: string) {
  const demoBanner = demoMode ? `<aside class="demo-banner" aria-label="Demo mode"><strong>Demo — sample data, nothing is saved to your real record.</strong><span>Demo actions never change your real record.</span><div><button class="banner-button" data-action="reset-demo">Reset demo</button><button class="banner-button" data-action="start-real">Start for real</button></div></aside>` : ''
...
function board() {
  ...
  return `<section class="intro"><div><p class="eyebrow">${online ? 'Local handoff · ready offline' : 'Offline · changes save on this device'}</p><h1 tabindex="-1">${dateHeading}</h1><p class="recipient">For <button class="text-button" data-action="person">${esc(data.personName || 'Care recipient')}</button></p><p class="intro-copy">Record the dose state first. Leave the detail clear enough for the next caregiver.</p></div><div class="date-stamp"><label for="date">Handoff date</label><input id="date" type="date" value="${selectedDate}" max="${today}" aria-describedby="date-help"><small id="date-help">Choose today or an earlier date.</small></div></section>
    <section class="shift-note" aria-labelledby="shift-heading"><div><p class="eyebrow">Shift note</p><h2 id="shift-heading">What should the next person know?</h2></div><textarea id="shift-note" maxlength="500" placeholder="Example: New blood pressure medication started today; hold only if the prescriber said so.">${esc(data.shiftNote)}</textarea><p class="quiet">This is a caregiver record, not medical advice. Confirm unclear instructions with the prescriber or pharmacist.</p></section>
...
function regimenChangeRow(change: AppData['regimenChanges'][number]): string {
  ...
  if (!change.previous.active && change.next.active) return `<li class="regimen-change"><b>Added to current list</b> ${esc(change.medicationName)} ${time}<span>${esc(change.next.dose)} · ${esc(change.next.slots.join(', '))}.</span></li>`
  if (change.previous.active && !change.next.active) return `<li class="regimen-change"><b>Removed from current list</b> ${esc(change.medicationName)} ${time}<span>Previously listed: ${esc(change.previous.dose)} · ${esc(change.previous.slots.join(', '))}. Existing dose history stays in this record.</span></li>`
...
document.addEventListener('click', async event => {
  ...
  if (action === 'remove-med') { const med = data.medications.find(m => m.id === target.dataset.id); if (med && confirm(`Remove ${med.name} from the current medication list? Existing dose history will stay.`)) { const changedAt = new Date().toISOString(); const removed = { ...med, active: false, changedAt }; const change = { id: crypto.randomUUID(), medicationId: med.id, medicationName: med.name, previous: { dose: med.dose, instructions: med.instructions, slots: med.slots, active: med.active }, next: { dose: removed.dose, instructions: removed.instructions, slots: removed.slots, active: removed.active }, changedAt }; message = `${med.name} was removed from the current list. Its history stays in this record.`; await save({ ...data, medications: data.medications.map(m => m.id === med.id ? removed : m), regimenChanges: [...data.regimenChanges, change] }) } }
