...
- Isolation: demo data is created in memory. Demo actions never write the real `med-handoff-card` IndexedDB database. Its appearance preference uses the separate `demo:mhc_theme` localStorage key; real mode uses `mhc_theme`.
