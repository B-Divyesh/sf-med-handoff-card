...
- Isolation: demo data is created in memory. Demo actions never open or write the real `med-handoff-card` IndexedDB database. Its optional appearance preference uses the separate `demo:mhc_theme` localStorage key; real mode uses `mhc_theme`.
