const VERSION = 'med-handoff-v1'
const SHELL = ['/', '/index.html', '/manifest.webmanifest', '/offline.html', '/privacy/index.html', '/terms/index.html', '/icons/icon-192.svg', '/icons/icon-512.svg']
...
self.addEventListener('fetch', event => {
  ...
  if (event.request.mode === 'navigate') {
    event.respondWith(fetch(event.request).then(response => { const copy = response.clone(); caches.open(VERSION).then(cache => cache.put('/index.html', copy)); return response }).catch(() => caches.match('/index.html').then(hit => hit || caches.match('/offline.html'))))
    return
  }
